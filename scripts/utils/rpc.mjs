 ...
 */
export function itemIgnoresCover(activity, coverId = "none") {
  // toDo: 
  ...
  if (props?.has?.("ignoreCover")) return { coverId: "none", bonus: 0 };

  if (actionType === "rwak" && coverId !== "total") {
    if (items?.getName("Sharpshooter") || items?.some(i => i.system?.identifier === "sharpshooter")) return { coverId: "none", bonus: 0 };
  }

  if (actionType === "rsak" && coverId !== "total") {
    if (items?.getName("Spell Sniper") || items?.some(i => i.system?.identifier === "spell-sniper")) return { coverId: "none", bonus: 0 };
  }

  const templateType = activity?.target?.template?.type ?? "";

  if (game.settings.get(MODULE_ID, SETTING_KEYS.IGNORE_ALL_AOE)) {
    if (templateType !== "") return { coverId: "none", bonus: 0 };
    ...
    ) return { coverId: "none", bonus: 0 };
  }

  if (game.settings.get(MODULE_ID, SETTING_KEYS.IGNORE_DISTANCE_SPACE)) {
    const rangeValue = activity?.range?.value ?? 0;
    if (rangeValue > 1 && (activity?.target?.affects?.type ?? "") === "space") return { coverId: "none", bonus: 0 };
  }

  return { coverId, bonus: COVER.BONUS[coverId] };
}